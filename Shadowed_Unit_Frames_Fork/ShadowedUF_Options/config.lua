...
local function getColor(info)
	local color = get(info) or {}
	-- Fall back to opaque white when a color is unset; some AceGUI-3.0 builds (shared via
	-- LibStub with other addons) error on a nil channel passed to SetVertexColor.
					...
					general = {
						...
						args = {
							...
							locale = {
								...
								type = "select",
								name = L["Language"],
								desc = L["Overrides the language used by Shadowed Unit Frames. Useful for languages the WoW client does not support. Changing this reloads your interface."],
								values = function()
									local list = {[""] = L["Client default"]}
									if( ShadowUF.localizedStrings ) then
										for code in pairs(ShadowUF.localizedStrings) do
											list[code] = ShadowUF.localeNames and ShadowUF.localeNames[code] or code
										end
									end
									return list
								end,
								get = function() return ShadowedUFLocale or "" end,
								set = function(info, value)
									ShadowedUFLocale = value ~= "" and value or nil
									ReloadUI()
								end,
							},
							sep = {
