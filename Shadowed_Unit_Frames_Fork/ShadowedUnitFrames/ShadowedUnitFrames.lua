...
-- API Compatibility layer for WoW 2.5.5+
...
-- Additional API compatibility for tag environment
local _GetRaidRosterInfo = C_RaidInfo and C_RaidInfo.GetRaidRosterInfo or GetRaidRosterInfo

local TagEnv = setmetatable({
	GetThreatStatusColor = ShadowUF.API.GetThreatStatusColor,
	GetRaidRosterInfo = _GetRaidRosterInfo,
}, { __index = _G, __newindex = function(k,v) _G[k] = v end })

-- Display names for the language selector. Non-Latin locales use ASCII names so the
-- dropdown stays legible on clients whose fonts cannot render those scripts.
ShadowUF.localeNames = {
	["deDE"] = "Deutsch",
	["esES"] = "Español (EU)",
	["esMX"] = "Español (LA)",
	["frFR"] = "Français",
...
-- Merges a manually selected locale on top of the active string table.
-- The selection lives in the account-wide saved variable ShadowedUFLocale rather than the
-- profile DB, so it can be read at PLAYER_LOGIN before the DB (and the defaults) are built.
-- The merge is one-way (into ShadowUF.L in place, which every module references), so
-- switching the locale off again requires a UI reload to restore the client default.
-- Unit labels and module names are copied out of L at file-load time, so they are rebuilt here.
function ShadowUF:ApplyLocale()
	local locale = ShadowedUFLocale
	if( not locale or not self.localizedStrings or not self.localizedStrings[locale] ) then return end

	-- Snapshot the base (untranslated) keys once, so rebuilds always resolve against L itself
	-- rather than an already-translated value if this ever runs more than once.
	if( not self.localeUnitKeys ) then
		self.localeUnitKeys = {}
		for token, name in pairs(self.L.units) do self.localeUnitKeys[token] = name end
	end
	if( not self.localeModuleKeys ) then
		self.localeModuleKeys = {}
		for key, module in pairs(self.modules) do self.localeModuleKeys[key] = module.moduleName end
	end

	for key, text in pairs(self.localizedStrings[locale]) do
		self.L[key] = text
	end

	-- Rebuild the load-time copies from the now-translated string table
	for token, key in pairs(self.localeUnitKeys) do
		self.L.units[token] = self.L[key]
	end
	for key, name in pairs(self.localeModuleKeys) do
		if( self.modules[key] ) then self.modules[key].moduleName = self.L[name] end
	end
end

function ShadowUF:OnInitialize()
	-- Apply any manually selected locale *before* defaults are built, so strings that get
	-- baked into the default profile (aura indicator names, text labels, ...) are localized too.
	self:ApplyLocale()
...
local hideRaidFrames

local hookedRehide = {}
local function hideProtectedFrame(frame, onShow)
	if( not frame ) then return end

	frame:Hide()

	-- Blizzard reparents the raid frames back to UIParent when it rebuilds them, so the
	-- parent has to be rechecked rather than assumed to still be our hidden frame
	if( frame:GetParent() ~= ShadowUF.hiddenFrame ) then
		frame:SetParent(ShadowUF.hiddenFrame)
	end

	if( not hookedRehide[frame] ) then
		hookedRehide[frame] = true
		frame:HookScript("OnShow", onShow or rehideFrame)
	end
end

-- The raid frames are recreated and reshown whenever the group changes, joining a battleground
-- and turning the group into a raid being the obvious case, so a single hide pass when the addon
-- loads is not enough. This reapplies the hide on every HideBlizzardFrames call, and reattaches
-- the frames to our hidden parent in case Blizzard moved them back under UIParent.
local hidingRaid
function hideRaidFrames()
	if( not CompactRaidFrameManager or hidingRaid ) then return end
	hidingRaid = true

	CompactRaidFrameManager:UnregisterAllEvents()
	if( CompactRaidFrameContainer ) then CompactRaidFrameContainer:UnregisterAllEvents() end

	-- Hiding or reparenting these is protected, in combat we can only kill the events and wait
	-- for PLAYER_REGEN_ENABLED to call back in and finish the job
	if( not InCombatLockdown() ) then
		-- These two run the hide again rather than the plain rehide, if Blizzard put them back
		-- under UIParent then hiding alone would just leave them to pop up on the next show
		hideProtectedFrame(CompactRaidFrameManager, hideRaidFrames)
		hideProtectedFrame(CompactRaidFrameContainer, hideRaidFrames)

		-- Group headers are created on demand as the raid fills up, the container being hidden
		-- already covers them, this is just so a stray show on one of them can't put it back up
		for i=1, MAX_RAID_GROUPS or 8 do
			hideProtectedFrame(_G["CompactRaidGroup" .. i])
		end
...
-- Blizzard reuses the arena enemy frames inside battlegrounds, Eye of the Storm putting a flag
-- carrier on ArenaEnemyFrame2 being one case, so both unit types hide the same set of frames
local hookedArena = {}
local function hideArenaFrame(frame)
	if( not frame ) then return end

	if( not hookedArena[frame] ) then
		hookedArena[frame] = true
...
-- Arena frames are secure unit buttons, so unregistering the unit watch and reparenting them is
-- blocked in combat. Entering a battleground mid-fight is the realistic case, PLAYER_REGEN_ENABLED
-- calls back in to finish the job once combat drops.
function ShadowUF:HideBlizzardArenaFrames()
	if( not self.enabledUnits.arena and not self.enabledUnits.battleground ) then return end
	if( InCombatLockdown() ) then return end

	hideArenaFrame(ArenaEnemyFrames)
	hideArenaFrame(ArenaPrepFrames)

	for i=1, MAX_ARENA_ENEMIES or 5 do
		local name = "ArenaEnemyFrame" .. i
		hideArenaFrame(_G[name])
		hideArenaFrame(_G[name .. "PetFrame"])
		hideArenaFrame(_G["ArenaPrepFrame" .. i])
	end
end
...
function ShadowUF:HideBlizzardFrames()
	...
	-- PartyFrame/PartyMemberFrameN are protected; touching them in combat raises
	-- "action blocked" errors. GROUP_ROSTER_UPDATE (which drives this) can fire in
	-- combat, so skip the party frames and let PLAYER_REGEN_ENABLED retry once combat ends.
	if( self.db.profile.hidden.party and not active_hiddens.party and not InCombatLockdown() ) then
		...
		if( PartyFrame ) then
			...
			-- Hook OnShow to immediately hide when Blizzard tries to show it (combat-safe)
	...
	if( self.db.profile.hidden.raid ) then
		if( not active_hiddens.raidTriggered and CompactRaidFrameManager ) then
			active_hiddens.raidTriggered = true

			-- Both of these run whenever Blizzard decides the raid frames should come back,
			-- UpdateShown on roster/zone changes and LayoutFrames when the frames are rebuilt
			if( CompactRaidFrameManager_UpdateShown ) then
				hooksecurefunc("CompactRaidFrameManager_UpdateShown", function()
					if( ShadowUF.db.profile.hidden.raid ) then hideRaidFrames() end
				end)
			end
	...
	-- party is left out on purpose, it sets its own flag once it actually managed to run,
	-- otherwise a roster update landing in combat would permanently skip the retry
	for type, flag in pairs(self.db.profile.hidden) do
		if( flag and type ~= "party" ) then
...
frame:SetScript("OnEvent", function(self, event, addon)
	...
	elseif( event == "ADDON_LOADED" and ( addon == "Blizzard_ArenaUI" or addon == "Blizzard_CompactRaidFrames" ) ) then
		...
		ShadowUF:HideBlizzardArenaFrames()
	elseif( event == "GROUP_ROSTER_UPDATE" or event == "PLAYER_REGEN_ENABLED" or event == "PLAYER_ENTERING_WORLD" or event == "ZONE_CHANGED_NEW_AREA" ) then
		if( not ShadowUF.db ) then return end

		-- Call HideBlizzardFrames to catch newly created party/raid frames.
		-- PLAYER_REGEN_ENABLED retries the hide once combat ends, in case a roster
		-- update landed while in combat and the protected-frame ops had to be skipped.
		if( ShadowUF.db.profile.hidden.party or ShadowUF.db.profile.hidden.raid ) then
			ShadowUF:HideBlizzardFrames()
		end

		-- Blizzard creates the arena frames on demand as opponents appear, so a single pass when
		-- the zone changed is not enough, and the pass is skipped outright while in combat
		ShadowUF:HideBlizzardArenaFrames()
	end
end)
